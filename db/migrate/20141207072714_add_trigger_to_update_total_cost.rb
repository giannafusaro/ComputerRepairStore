class AddTriggerToUpdateTotalCost < ActiveRecord::Migration
  def up
    execute <<-SQL
      CREATE TRIGGER update_part_cost_on_update AFTER UPDATE ON parts
      FOR EACH ROW
      BEGIN
    ...
    execute <<-SQL
      CREATE TRIGGER update_part_cost_on_insert AFTER INSERT ON parts
      FOR EACH ROW
      BEGIN
        DECLARE parts_total DECIMAL(8,2) DEFAULT 0;
        DECLARE labor_total DECIMAL(8,2) DEFAULT 0;
  ...
  def down
    execute "DROP TRIGGER update_part_cost_update;"
    execute "DROP TRIGGER update_part_cost_insert;"
  end
end
